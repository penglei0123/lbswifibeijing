...
public class RegexUtil {
	 ...
	 * 字母\数字\下划线
	 */
	public static Pattern	regCharNumberPattern	= Pattern.compile("^\\w{6,20}$");
	public static Pattern	regTokenPattern			= Pattern.compile("^[A-Za-z0-9-]+$");
	...
	public static boolean isVaildToken(String token) {
		return regTokenPattern.matcher(token).matches();
	}
	...
	public final static boolean isUserName(String str) {
		return regCharNumberPattern.matcher(str).matches();
	}

	/**
	 * 匹配密码，以字母开头，长度在6-15之间，只能包含字符、数字和下划线。
	 * 
	 * @param str
	 * @return
	 * @author jiqinlin
	...
	public final static boolean isPwd(String str) {
		if (str == null)
			return false;
		return match(str, "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,15}$");
	}
	 ...
	 * @deprecated
	 */
	public final static boolean isEmailOd(String str) {
		return match(str, "^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+.([a-zA-Z0-9_-])+$");
	}

	public final static boolean isEmail(String str) {
		return match(str, "[\\w\\.\\-]+@([\\w\\-]+\\.)+[\\w\\-]+");
	}
