...
//获取ap的位置，显示在地图上
function getAllApPosition(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		...
		success : function(result){
			if (result.status !== 1){
				...
				for ( var i in data){
					...
					var canvasbuffer = document.createElement('canvas');
					var contextbuffer = canvasbuffer.getContext('2d');
					drawApPosition(canvas, context, canvasbuffer, contextbuffer, i, data[i]);
				}
			}
		},
		error : function(){
...
function drawApPosition(canvas, context, canvasbuffer, contextbuffer, floor, positions){
	var img = new Image();
	...
	img.onload = function(){
		...
		context.drawImage(img, 0, 0, img.width * canvasScale, img.height
				* canvasScale);
